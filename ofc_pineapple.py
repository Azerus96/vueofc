...
# Версия с НАЧАЛОМ реализации Fantasyland (v15 - вход и первая улица N)
...
class OFCPineappleState(pyspiel.State):
    ...
    # ИЗМЕНЕНО v12: Добавлена обработка фаз Fantasyland N
    ...
    # ИЗМЕНЕНО v12: Добавлена обработка фаз Fantasyland
    ...
    def information_state_string(self, player: int) -> str:
        ...
        if self._is_fantasy_hand:
            f_status = f"CurF:{self._current_fantasy_player};CurN:{self._current_normal_player}"
        else:
            fantasy_next = [int(p in self._next_fantasy_players) for p in range(self._num_players)]
            f_status = f"NextF:[{fantasy_next[0]}{fantasy_next[1]}]"
